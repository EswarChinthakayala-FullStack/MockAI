...
def check_app_module() -> tuple[bool, str]:
    try:
        ...
        return result.returncode == 0, "app.main importable"
    except Exception as e:
        return False, str(e)
...
def check_celery_app_module() -> tuple[bool, str]:
    try:
        result = subprocess.run(
            [sys.executable, "-c", "import app.tasks.celery_app; print('ok')"],
            capture_output=True, text=True, timeout=8
        )
        return result.returncode == 0, "app.tasks.celery_app importable"
    except Exception as e:
        return False, str(e)
